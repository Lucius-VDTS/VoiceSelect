...
public class JSONLayout {
    ...
    public JSONLayout(Layout layout, List<LayoutColumn> layoutColumns, List<Column> columns , List<VDTSUser> users) {
        ...
        //displayCode = layout.getNameCode();
        ...
        for (LayoutColumn lc : layoutColumns){
            List<Column> col = columns.stream()
                    .filter(c -> c.getUid() == lc.getColumnID()).collect(Collectors.toList());
            if (col.size()>0){
                this.layoutColumns.add(new JSONLayoutColumn(col.get(0).getExportCode(),lc.getColumnPosition()));
            }
        }
    }
